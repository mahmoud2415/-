...
const branches = [
  ...
  },
  {
    ...
    mapLink:
      "https://www.google.com/maps/search/%D8%AD%D9%84%D9%88%D8%A7%D9%86%D9%8A%20%D8%A7%D9%84%D8%A3%D8%B3%D8%B7%D9%88%D8%B1%D8%A9/@30.73116373,31.79502818,17z?hl=ar",
  },
  {
    id: "faqous-kafr",
    ...
    mapLink:
      "https://www.google.com/maps/search/9A%20cafe%20and%20food%20court/@30.72620635,31.78810442,17z?hl=ar",
  },
  {
    id: "abukibir",
    name: "فرع أبو كبير",
...
function getCartItemImage(item, index = 0) {
  ...
  const productFromName = menuData.find(
    (menuItem) => item.name && item.name.includes(menuItem.name),
  );
    ...
    .map((section) => {
      ...
      const itemsHTML = sectionItems
        .map((item) => {
          ...
          return `
                <div class="menu-product-card overflow-hidden group flex flex-col cursor-pointer"
                     style="max-height:22rem;"
                     onclick='openCustomizationModal(${JSON.stringify(item)})'>
                    <div class="relative w-full overflow-hidden bg-[#f8ece7] m-3 mb-0 rounded-[20px]"
                    ...
                    <div class="p-3 sm:p-4 flex-1 min-w-0 flex flex-col justify-between gap-2 sm:gap-2">
                        <div>
                            <div class="flex justify-between items-start gap-2">
                                <h4 class="font-bold text-sm text-[#1c1b1b] leading-tight group-hover:text-primary transition-colors duration-300">${item.name}</h4>
                                <span class="font-bold text-primary text-sm whitespace-nowrap">${priceDisplay}</span>
                            </div>
                            <p class="text-[10px] text-on-surface-variant/80 line-clamp-2 leading-snug mt-1">${item.desc || "وجبة زنجر المميزة بنكهتها الخاصة وقرمشتها الفريدة."}</p>
                        </div>
                        <button type="button" aria-label="افتح تخصيص ${item.name}" onclick='event.stopPropagation(); openCustomizationModal(${JSON.stringify(item)})' class="w-full py-2 bg-primary rounded-2xl flex items-center justify-center text-white text-[10px] font-black gap-1 shadow-[0_8px_20px_rgba(173,43,0,0.2)] transition-all duration-300 hover:scale-[1.01] focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
                          <span>أضف للسلة</span>
  ...
  if (!slider.dataset.listenerAttached) {
    slider.addEventListener("scroll", () => {
      ...
      if (
        ...
        index >= 0 &&
        index < currentModalImages.length
      ) {
    ...
    for (let i = 0; i < currentModalImages.length; i++) {
      ...
      if (dot) {
        if (i === currentModalImageIndex) {
          dot.className =
            "w-2.5 h-2.5 rounded-full transition-all duration-300 bg-white scale-125";
        } else {
          dot.className =
            "w-2.5 h-2.5 rounded-full transition-all duration-300 bg-white/50";
        }
      }
    }
  }
...
function showModalImage(index) {
  ...
  currentModalImageIndex =
    (index + currentModalImages.length) % currentModalImages.length;
...
function checkout() {
  ...
  // Prepare message
  let msg = `*طلب جديد من موقع مطعم زنجر* 🍔%0a%0a`;
  msg += `*الفرع:* ${activeBranchName}%0a`;
  msg += `*الاسم:* ${name}%0a`;
  ...
  cart.forEach((item) => {
    ...
    subtotal += itemTotal;
    msg += `• ${item.name} x${item.qty} = ${itemTotal} ج.م%0a`;
  });
...
// 7. Keep header and categories nav visible while scrolling
// The fixed header and sticky categories navigation should remain visible at the top of the page.
...
function updateBranchUI() {
  ...
  const footerBranchAddressEl = document.getElementById(
    "footer-branch-address",
  );
  if (footerBranchAddressEl) {
    footerBranchAddressEl.innerText = selectedBranch.address;
  }
  const footerBranchMapLinkEl = document.getElementById(
